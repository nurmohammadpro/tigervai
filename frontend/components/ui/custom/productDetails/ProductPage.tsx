    ...
    <section className="relative">
      <div className="space-y-1">
        ...
        <button
          ...
          className="w-full py-3 px-2 md:px-4 rounded-full font-semibold text-sm md:text-base transition-all bg-[#2E83F2] hover:bg-[#1976D2] text-white flex items-center justify-center gap-1 md:gap-2 disabled:opacity-50"
        >
          {isChatPending ? (
        ...
        >
          ...
          <span className="truncate">Chat Via WhatsApp</span>
        </a>
      </div>
    </section>
  );
...
const ProductPage = ({ params }: { params: Product }) => {
  ...
  const parseCartItemIdTyre = (id: string) => {
    const parts = id.split("|");
    if (parts.length < 3) return null;
    return {
      productId: parts[0],
  ...
  const getVariantPriceTyre = (variant: (typeof params.variants)[0]) => {
    if (variant.discountPrice && variant.discountPrice > 0) {
      return {
        originalPrice: variant.price,
        currentPrice: variant.discountPrice,
  ...
  const handleAddAllToCartTyre = () => {
    ...
    Object.entries(variantQuantities).forEach(([cartItemId, quantity]) => {
      if (quantity > 0) {
        const parsed = parseCartItemIdTyre(cartItemId);
        if (!parsed) return;
          ...
          <div className=" ">
            ...
            <div className="">
              ...
              {params?.special_offer && (
                <div className="flex items-center gap-2 bg-green-50/20 rounded-lg p-0 py-1">
            ...
            {params?.variants && params.variants.length > 0 && (
              <div className="space-y-4">
                ...
                {/* Divider with OR */}
                <div className="relative flex items-center -mt-2 mb-1.5">
